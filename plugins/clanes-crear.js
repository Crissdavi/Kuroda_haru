...
const handler = async (m, { conn, text, usedPrefix, command, args }) => {
  ...
  if (user.level < 15) {
    if (!text) return m.reply(`⚠️ Para poder crear un clan es necesario que seas nivel 15.\n\nNivel actual: ${user.level}`);
  }

  if (!args[0] || args[0].trim().length === 0) {
    if (!text) return m.reply(`*[ 🌱 ] Por favor ingrese un nombre válido para su clan.*`);
  }

  const clanExistente = await verificarClanExistente(m.sender);
  if (clanExistente) {
    if (!text) return m.reply(`⚠️ Ya eres líder de un clan llamado "${clanExistente.nombre}". No puedes crear otro clan.`);
  }
  ...
  async function guardarClan() {
    ...
    try {
      ...
      if (!text) return m.reply(`⚠️ Hubo un error al crear el clan. Por favor, intente de nuevo más tarde.`);
    }
  }
...
async function verificarClanExistente(lider) {
  try {
    ...
    if (!text) return m.reply(`⚠️ Hubo un problema al verificar si ya tienes un clan. Intenta de nuevo más tarde.`);
  }
}