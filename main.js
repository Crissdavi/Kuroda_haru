...
global.authFile = `Sesion Principal`;
...
do {
let lineM = '⋯ ⋯ ⋯ ⋯ ⋯ ⋯ ⋯ ⋯ ⋯ ⋯ ⋯ 》'
opcion = await question(chalk.bgBlack(chalk.bold.greenBright(`[ ℹ️ ] Seleccione una opción:\n1. Con código QR\n2. Con código de texto de 8 dígitos\n---> `)))
if (!/^[1-2]$/.test(opcion)) {
...
if (!opts['test']) {
  if (global.db) {
    setInterval(async () => {
      ...
      if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', 'bebots'], tmp.forEach((filename) => cp.spawn('find', [filename, '-amin', '3', '-type', 'f', '-delete'])));
    }, 30 * 1000);
  }
}
