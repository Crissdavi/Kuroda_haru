...
global.authFile = 'KurodaSession'
global.authFileJB  = 'KurodaJadiBot'
...
│ ${chalk.blueBright('╰┄┈┅┈┄┈┅┈┄┅┈┄┈┅┄┈┅┈┄')} 
│ ${chalk.italic.red(`Buenos dias , buenas tardes o buenas noches 🌷`)}
...
mobile: MethodMobile, 
browser: opcion == '1' ? ['Kuroda', 'Edge', '20.0.04'] : methodCodeQR ? ['Kuroda', 'Edge', '20.0.04'] : ["Ubuntu", "Chrome", "20.0.04"],
auth: {
creds: state.creds,
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "KorudaJadiBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
console.log('🚩 Kuroda no tiene Sub-Bots vinculados.');
return;
}
...
const mainBotAuthFile = 'KurodaSession';
try {
...
console.error(chalk.bold.cyanBright(` Error al iniciar Koruda: `, error))
}
})();
...
let prekey = []
let directorio = readdirSync("./KorudaSession")
...
const directories = ['./KurodaSession/', './KurodaJadiBot/']
